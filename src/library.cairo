...
// Based on OpenZeppelin Contracts for Cairo v0.6.0 (account/library.cairo)
...
@storage_var
func Account_whitelisted_contract_addresses(address: felt) -> (is_allowed: felt) {
}
...
namespace Account {
    ...
    func assert_only_whitelisted_contracts{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr} (
        to_address: felt
    ) {
        // We allow calling the account contract for setup purposes
        // (no worries, reentrancy is blocked at __execute__)
        let (self) = get_contract_address();
        if (self == to_address) {
            return ();
        }

        let (allowed) = Account_whitelisted_contract_addresses.read(to_address);
        with_attr error_message("contract not whitelisted") {
            assert allowed = 1;
        }
    ...
    func set_whitelisted_contract{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr} (
        contract_address: felt, allowed: felt
    ) {
        assert_only_self();
        assert_le_felt(allowed, 1);
        Account_whitelisted_contract_addresses.write(contract_address, allowed);
    ...
    func _execute_list{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        // check that the contract is whitelisted
        assert_only_whitelisted_contracts(this_call.to);
